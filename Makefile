BINARY   := homepage
COUNT    := homepage-count.txt
ADDR     := :8080
BIN_DIR  := bin
PID_FILE := $(BIN_DIR)/$(BINARY).pid
LOG_FILE := $(BIN_DIR)/$(BINARY).log

# What 'make start' polls to decide the daemon is actually serving. /healthz is
...
run:
	go run . -addr $(ADDR) -counter $(COUNT)
...
start: build
	@if [ -f $(PID_FILE) ] && kill -0 `cat $(PID_FILE)` 2>/dev/null; then \
		...
		nohup $(BIN_DIR)/$(BINARY) -addr $(ADDR) -counter $(COUNT) > $(LOG_FILE) 2>&1 & \
		pid=$$!; \
		echo $$pid > $(PID_FILE); \
		i=0; \
		while [ $$i -lt 100 ]; do \
...
# TERM rather than the default KILL: the count is flushed on SIGTERM, and
# killing outright would throw away every visit since the last tick.
stop:
	@if [ -f $(PID_FILE) ] && kill -0 `cat $(PID_FILE)` 2>/dev/null; then \
